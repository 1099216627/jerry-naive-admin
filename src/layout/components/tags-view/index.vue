...
import { RouteItem } from "@/stores/modules/tags-view";
...
import { useDesignSetting } from "@/hooks/settings/use-setting-hook";
...
// 初始化标签页
tagsViewStore.initTabs(cacheRoutes);
...
const removeTab = (route) => {
  ...
  tagsViewStore.closeCurrentTab(route);
  // 如果关闭的是当前页
...
//   tagsViewStore.closeLeftTabs(route);
...
//   tagsViewStore.closeRightTabs(route);
//   state.activeKey = route.fullPath;
//   router.replace(route.fullPath);
//   updateNavScroll();
// };
...
const closeOther = (route) => {
  tagsViewStore.closeOtherTabs(route);
  state.activeKey = route.fullPath;
  router.replace(route.fullPath);
  updateNavScroll();
};
